...
export async function deduplicateArticles(articles: EnrichedArticle[]): Promise<EnrichedArticle[]> {
  ...
  const prompt = `你是一个 AI 新闻去重助手。以下是一组 AI 新闻标题，请找出**围绕同一核心事件**的文章组（哪怕角度不同也算）。

规则：
- 报道同一具体事件算重复（例如"OpenAI 发布 GPT-5"和"GPT-5 正式推出"是重复的）
- **同一核心事件的不同侧面/评论/分析也算重复**——例如以下三种情况都算同一组：
  - "Anthropic 完成 H 轮融资 650 亿"（新闻）
  - "Anthropic 估值是否合理"（评论）
  - "Anthropic 营收数据曝光"（侧面数据）
